 ...
 */
 ...
 {
   std::string buf;
   yfs_client::status ret = yfs->read(ino, buf, size, off);
    if (ret != yfs_client::OK) {
      fuse_reply_err(req, ENOENT);
      return;
    }
   const char* buf_cstr = buf.c_str();
   fuse_reply_buf(req, buf_cstr, size);
 }
   ...
   struct fuse_file_info *fi)
 {
   ...
   yfs_client::status ret = yfs->write(ino, buf, size, off, bytes_written);
   if (ret != yfs_client::OK) {
     fuse_reply_err(req, ENOENT);
     return;
   }
 ...
 {
  printf("fuseserver_createhelper %llu %s\n", parent, name);
  if (!yfs->isdir(parent)) {
  ...
  if (inum == 0) {
    ...
    yfs_client::status ret = yfs->createfile(parent, name, new_inum);
 ...
 {
    printf("fuseserver_create %llu %s\n", parent, name);
 ...
 fuseserver_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
 {
   ...
   e.attr_timeout = 0.0;
   e.entry_timeout = 0.0;
   printf("fuseserver_lookup %llu %s\n", parent, name);
   // You fill this in:
   // Look up the file named `name' in the directory referred to by
   // `parent' in YFS. If the file was found, initialize e.ino and
   // e.attr appropriately.

   // call yfs -> ilookup to get the inum of the file with name in directory of ino parent
   yfs_client::inum inum = yfs->ilookup(parent, name);
    if (inum != 0) {
      found = true;
      e.ino = inum;
           ...
           off_t off, struct fuse_file_info *fi)
 {
  ...
  std::vector<yfs_client::dirent> dir_files;
  yfs_client::status ret = yfs->readdir(inum, dir_files);
    // fill in the b data structure using dirbuf_add
  for (std::vector<yfs_client::dirent>::iterator it = dir_files.begin(); it != dir_files.end(); ++it) {
    e = *it;
